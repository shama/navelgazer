...
// Globals for linux rename support
var linuxRenameWaiting = null;
var linuxRenamePath = null;
function clearLinuxRename() {
  clearTimeout(linuxRenameWaiting);
  linuxRenameWaiting = null;
  linuxRenamePath = null;
}
...
HandleWatcher.prototype.onEvent = function(event, filePath, oldFilePath) {
  ...
  switch (event) {
    ...
    case 'delete':
      ...
      return this.close();
    // Check for a potential rename event on linux
    case 'linux-renamein':
      var newPath = path.resolve(self.path, oldFilePath);
      if (linuxRenameWaiting) {
        console.log('RENAMEIN/YES', newPath, linuxRenamePath)
        self.emit('change', 'rename', newPath, linuxRenamePath);
        clearLinuxRename();
      } else {
        console.log('RENAMEIN/NO', newPath, linuxRenamePath)
        linuxRenamePath = newPath;
        linuxRenameWaiting = setTimeout(function() {
          // If no subsequent event, change event on folder
          self.emit('change', 'change', filePath, oldFilePath);
          clearLinuxRename();
        }, 100);
      }
      break;
    case 'linux-renameout':
      if (linuxRenameWaiting) {
        console.log('RENAMEOUT/YES', linuxRenamePath, self.path)
        self.emit('change', 'rename', linuxRenamePath, self.path);
        clearLinuxRename();
      } else {
        console.log('RENAMEOUT/NO', linuxRenamePath, self.path)
        linuxRenamePath = self.path;
        linuxRenameWaiting = setTimeout(function() {
          // If no subsequent event, consider this file deleted
          self.emit('change', 'delete', null);
          self.close();
          clearLinuxRename();
        }, 100);
      }
