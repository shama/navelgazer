    ...
    setTimeout(function() {
      ...
      try {
        return done(new PathWatcher(path.resolve(file), function(event, newFile) {
          var filepath = file;
          console.log('hit', event, filepath)
          if (process.platform === 'linux') {
            var go = linuxWorkarounds(event, filepath, cb);
            if (go === false) { return; }
          }
...
function linuxProcessQueue(cb) {
  ...
  if (len === 1) {
    ...
    // If change and delete events fired simultaneously, use delete
    if (linuxQueue['delete'] && linuxQueue['change']) {
      console.log('HIT WORKAROUND', linuxQueue)
      cb(null, 'delete', linuxQueue['delete']);
    }
  }
